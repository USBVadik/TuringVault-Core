...
async function main() {
  ...
  assert(
    "validationRegistry",
    DEFAULT_CONTRACTS.validationRegistry ===
      "0x6841d3DAF81A446C8Bd6934F7516f2Ee1b4d63b6"
  );
  assert(
    "decisionLog",
    DEFAULT_CONTRACTS.decisionLog ===
      "0x7bCd905678ed5dB1e87852b933f1aEfE544cfbB5"
  );
  assert(
    "identity",
    DEFAULT_CONTRACTS.identity === "0x6f862802e0d5463DF18d267e422347BeCacc28bD"
  );
  assert(
    "reputation",
    DEFAULT_CONTRACTS.reputation ===
      "0xC78119F3274B05046Ac7c38a14298a6cbD946e1a"
  );
  ...
  try {
    ...
    if (proposals.length > 0) {
      ...
      assert(
        "has status field",
        ["Pending", "Approved", "Rejected", "Expired"].includes(p.status),
        p.status
      );
      assert("has riskScore", typeof p.riskScore === "number");
      assert(
        "has validatorReasoning",
        typeof p.validatorReasoning === "string"
      );
    }
  } catch (e) {
  ...
  try {
    ...
    if (agent && agent.name) {
      ...
      assert(
        "has IPFS cid",
        typeof agent.cid === "string",
        agent.cid.slice(0, 20) + "..."
      );
    }
  } catch (e) {
  ...
  try {
    await sdk.createValidatedDecision({
      analyst: { action: "swap", confidence: 0.8 },
      validator: { riskScore: 30 },
    });
    assert("requires private key", false, "should have thrown");
  } catch (e) {
    assert(
      "requires private key",
      e.message.includes("Private key required"),
      e.message
    );
  }
