...
class TuringVaultSDK {
  ...
  async createValidatedDecision(params) {
    ...
    const proofDocument = {
      ...
      consensus: {
        reached: (validator.approved !== false) && analyst.confidence >= 0.6 && validator.riskScore <= 65,
    ...
    if (this.pinataJwt) {
      ...
      ipfsCid = 'sha256:' + crypto.createHash('sha256').update(JSON.stringify(proofDocument)).digest('hex');
    }
    
    const confidenceBps = Math.round(analyst.confidence * 10000);
    const validatorConfidenceBps = Math.round((validator.confidence || analyst.confidence) * 10000);
    const riskScoreBps = validator.riskScore * 100; // 0-10000

    // Step 2: Submit proposal to ValidationRegistry
    const registry = new ethers.Contract(this.contracts.validationRegistry, VALIDATION_REGISTRY_ABI, this.wallet);
    ...
    const totalGas = proposalReceipt.gasUsed + validateTx.gasUsed + logReceipt.gasUsed;
  ...
  async logDecision(params) {
    ...
    const { action, targetAsset = 'ETH', amountIn = 0, amountOut = 0, confidence, reasoningHash } = params;
    
    const decisionLog = new ethers.Contract(this.contracts.decisionLog, DECISION_LOG_ABI, this.wallet);
    const confidenceBps = Math.round((confidence || 0) * 10000);
    
    const tx = await decisionLog.logDecision(
      action, targetAsset, amountIn, amountOut, confidenceBps,
      reasoningHash || '', ethers.ZeroHash,
    );
    const receipt = await tx.wait();
    const decisionId = Number(await decisionLog.totalDecisions()) - 1;
    
    return { decisionId, txHash: receipt.hash, gasUsed: receipt.gasUsed.toString() };
  }
  ...
  async getConsensusRate() {
    const registry = new ethers.Contract(this.contracts.validationRegistry, VALIDATION_REGISTRY_ABI, this.provider);
    const [approved, rejected, total] = await registry.getConsensusRate();
    return { approved: Number(approved), rejected: Number(rejected), total: Number(total) };
  }
  ...
  async getAgentIdentity(agentId = 0) {
    const identity = new ethers.Contract(this.contracts.identity, IDENTITY_ABI, this.provider);
