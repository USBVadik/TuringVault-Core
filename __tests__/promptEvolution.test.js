 ...
 */
const { PromptEvolution, EVOLUTION_CONFIG } = require("../src/evolution/promptEvolution");
const fs = require("fs");
...
describe("Prompt Evolution", () => {
  ...
  describe("shouldEvolve", () => {
    ...
    test("returns false when too few decisions", () => {
      const result = evo.shouldEvolve({ totalDecisions: 3, score: -100, totalFeedback: 0 });
      expect(result.should).toBe(false);
      expect(result.reason).toContain("Need");
    });

    test("returns true when enough decisions and seeking optimization", () => {
      const result = evo.shouldEvolve({ totalDecisions: 15, score: 10, totalFeedback: 5 });
      expect(result.should).toBe(true);
      expect(result.reason).toContain("optimization");
    });

    test("returns true on poor performance", () => {
      const result = evo.shouldEvolve({ totalDecisions: 20, score: -100, totalFeedback: 10 });
      expect(result.should).toBe(true);
    ...
    test("respects cooldown period", () => {
      ...
      const result = evo.shouldEvolve({ totalDecisions: 50, score: -200, totalFeedback: 20 });
      expect(result.should).toBe(false);
  ...
  describe("EVOLUTION_CONFIG", () => {
    test("has sane defaults", () => {
      expect(EVOLUTION_CONFIG.minDecisionsForReflection).toBeGreaterThanOrEqual(5);
