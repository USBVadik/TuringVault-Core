...
function runBacktest(prices, config = {}) {
  ...
  for (let i = lookback; i < prices.length; i++) {
    ...
    if (position) {
      ...
      if (position.type === "long") {
        ...
        // Trailing stop
          ...
          if (
            profitPct >= trailingActivation &&
            position.hwm > position.entry
          ) {
            const trailLevel =
              position.hwm -
              (position.hwm - position.entry) * (1 - trailingKeep);
            if (price <= trailLevel) {
        ...
        if (price >= position.sl) {
          ...
          if (
            ...
            const trailLevel =
              position.hwm +
              (position.entry - position.hwm) * (1 - trailingKeep);
            if (price >= trailLevel) {
              exitReason = "TRAILING_STOP";
            }
          }
    ...
    if (pos <= buyZone) {
      ...
      const slDistance = Math.max(
        price * minSlPct,
        expectedReward * slRewardFraction
      );
  ...
  if (position) {
    ...
    trades.push({
      ...
      pnlPct: +pnlPct.toFixed(4),
      reason: "END",
      bars: prices.length - position.startBar,
    });
  }
...
if (require.main === module) {
  ...
  console.log(
    `  Ranging (tight):  ${
      result1.totalPnlPct > 0 ? "✅" : "❌"
    } ${result1.totalPnlPct.toFixed(2)}% over ${result1.totalTrades} trades`
  );
  console.log(
    `  Ranging (medium): ${
      result2.totalPnlPct > 0 ? "✅" : "❌"
    } ${result2.totalPnlPct.toFixed(2)}% over ${result2.totalTrades} trades`
  );
  console.log(
    `  Ranging (wide):   ${
      result3.totalPnlPct > 0 ? "✅" : "❌"
    } ${result3.totalPnlPct.toFixed(2)}% over ${result3.totalTrades} trades`
  );
  console.log(
    `  Trending (bad):   ${
      result4.totalPnlPct > 0 ? "✅" : "❌"
    } ${result4.totalPnlPct.toFixed(2)}% over ${result4.totalTrades} trades`
  );
  console.log(
    `\n  Strategy is profitable in ranging = ${
      result1.totalPnlPct > 0 &&
      result2.totalPnlPct > 0 &&
      result3.totalPnlPct > 0
        ? "✅ YES"
  ...
  console.log(
    `  Strategy loses in trending = ${
      result4.totalPnlPct < 0
        ? "✅ EXPECTED (regime filter protects)"
        : "⚠️  Also profitable (unexpected)"
    }`
  );
}
...
function printResult(r) {
  ...
  console.log(
    `  Win rate: ${r.winRate}% | Avg PnL/trade: ${r.avgPnlPct.toFixed(4)}%`
  );
  console.log(
    `  Total PnL: ${r.totalPnlPct.toFixed(4)}% | Max drawdown: ${
      r.maxDrawdown
    }%`
  );
