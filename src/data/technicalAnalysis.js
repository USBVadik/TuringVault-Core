...
function calcMACD(prices) {
  ...
  const histogram = macdLine
    .slice(26)
    .map((v, i) => (i < signalLine.length ? v - signalLine[i] : 0));
  return {
    macdLine: macdLine.slice(-5),
    signalLine: signalLine.slice(-5),
    histogram: histogram.slice(-5),
  };
}
...
function calcBollingerBands(prices, period = 20) {
  ...
  const stdDev = Math.sqrt(
    recent.reduce((s, p) => s + (p - mean) ** 2, 0) / period
  );
  return {
    upper: mean + 2 * stdDev,
    middle: mean,
...
async function getTechnicalAnalysis(coin = "ethereum") {
  ...
  if (lastHist > 0 && prevHist <= 0)
    ...
    signals.push({ indicator: "MACD", signal: "BULLISH", strength: "BUY" });
  ...
  if (bollinger) {
    ...
    else if (bollinger.position > 0.9)
      signals.push({
        indicator: "BOLLINGER",
  ...
  const totalScore = signals.reduce(
    (s, sig) => s + (scoreMap[sig.strength] || 0),
    0
  );
  ...
  return {
    ...
    indicators: {
      rsi: {
        value: rsi.toFixed(1),
        prev: rsiPrev?.toFixed(1),
        trend: rsi > rsiPrev ? "rising" : "falling",
      },
      ema: {
        ema9: ema9Now.toFixed(2),
        ema21: ema21Now.toFixed(2),
        trend: ema9Now > ema21Now ? "bullish" : "bearish",
      },
      macd: {
        histogram: lastHist.toFixed(4),
        trend: lastHist > prevHist ? "improving" : "weakening",
      },
      bollinger: bollinger
        ? {
...
function getSummary(signal, rsi, ema9, ema21, score, price) {
  ...
  parts.push(
    `EMA9 ${ema9 > ema21 ? "above" : "below"} EMA21 (${
