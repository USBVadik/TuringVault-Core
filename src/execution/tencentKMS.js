...
class TencentKMSSigner extends ethers.AbstractSigner {
  ...
  constructor(options) {
    ...
    if (this._useKMS) {
      console.log("[KMS] Tencent Cloud KMS signing enabled (key:", options.keyId?.slice(0, 8) + "...)");
    } else {
  ...
  async getAddress() {
    ...
    throw new Error("KMS getAddress requires public key derivation — configure keyId");
  }
  ...
  async _kmsSign(digest) {
    ...
    console.log(`[KMS] Would sign digest ${digest.slice(0, 18)}... via Tencent KMS key ${this.keyId}`);
    console.log(`[KMS] API: POST https://kms.tencentcloudapi.com Action=AsymmetricSign`);
    ...
    throw new Error("Production KMS signing not yet implemented — contact Tencent Cloud support for EVM key provisioning");
  }
}
