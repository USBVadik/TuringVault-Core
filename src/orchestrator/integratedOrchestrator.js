...
const INTENT_QUEUE_PATH = path.resolve(
  __dirname,
...
function calculateVaR(marketData, decision) {
  ...
  // Action type premium
  if (decision.analyst?.action === "swap" || decision.analyst?.action === "buy")
    var_bps += 30;
...
async function runIntegratedCycle(options = {}) {
  ...
  const wallet = new ethers.Wallet(
    (process.env.PRIVATE_KEY?.startsWith("0x") ? "" : "0x") +
  ...
  const dex = new OpenOceanDEX(provider, wallet, {
    dryRun: mode !== "autonomous",
  });
  const dexQuoter = new MerchantMoeDEX({
  ...
  console.log(
    `   ETH: $${market.ethPrice} (${
      market.ethChange24h > 0 ? "+" : ""
    }${market.ethChange24h?.toFixed(1)}%) | MNT: $${market.mntPrice}`
  );
  console.log(
    `   Sentiment: ${market.sentiment} (F&G: ${
      market.fearGreedIndex
    }) | TVL: $${((market.mantleTVL || 0) / 1e6).toFixed(0)}M\n`
  );
  ...
  console.log(
    `   USDY: ${rwaPosition.balance.toFixed(4)} ($${rwaPosition.balance.toFixed(
      2
    )}, ${rwaPosition.apy}% APY)`
  );
  ...
  try {
    ...
    console.log(
      `   DEX: 1 MNT = $${mntQuote.estimatedOut?.toFixed(
        4
      )} USDT (impact: ${mntQuote.priceImpact?.toFixed(1)}%)\n`
    );
  } catch (e) {
  ...
  const allocation = await rwa.calculateAllocation(
    {
      ...balances,
      USDY: rwaPosition.balance,
      MNT_PRICE: market.mntPrice || 0.62,
    },
    {
      volatilityIndex: Math.min(
        100,
        Math.abs(market.ethChange24h || 0) * 20 + 30
      ),
  ...
  console.log(
    `   Portfolio: $${allocation.totalPortfolioValue.toFixed(
      2
    )} | RWA target: ${allocation.targetRWAAllocation.toFixed(1)}%`
  );
  console.log(
    `   Action: ${allocation.action} | Yield: $${allocation.yieldProjection.annual}/yr\n`
  );
  ...
  const enrichedMarket = {
    ...
    promptContext: [
      ...
      `\n[DEX] Merchant Moe: 1 MNT = $${
        mntQuote?.estimatedOut?.toFixed(4) || "N/A"
      } USDT (LB v2.1, binStep 15)`,
      `[RWA] USDY position: $${rwaPosition.balance.toFixed(
        2
      )} | Target allocation: ${allocation.targetRWAAllocation.toFixed(
        0
      )}% | Action: ${allocation.action}`,
      `[PORTFOLIO] Total: $${allocation.totalPortfolioValue.toFixed(
        2
      )} | MNT: ${balances.MNT?.toFixed(2)}`,
    ].join("\n"),
  };
  ...
  console.log(
    `   VaR: ${var_bps} bps | Threshold: ${CONFIG.varThreshold.autonomous}/${CONFIG.varThreshold.supervised}/${CONFIG.varThreshold.blocked}`
  );
  ...
  if (mode === "supervised" && autonomyLevel === "SUPERVISED") {
    ...
    const intent = await intentQueue.addIntent({
      ...
      allocation,
      dexQuote: mntQuote
        ? { price: mntQuote.price, impact: mntQuote.priceImpact }
        : null,
    });
  ...
  if (autonomyLevel === "BLOCKED") {
    console.log(
      "   ⛔ VaR too high — no execution. Recording observation only.\n"
    );
  }
  ...
  console.log(
    `   KMS signature: r=${kmsSig.r.slice(0, 14)}... v=${kmsSig.v} (${
      kmsSig.simulated ? "simulated" : "hardware"
    })`
  );
  ...
  const tx1 = await registry.submitProposal(
    ...
    confidenceBps,
    `[v2] ${
      decision.analyst?.reasoning?.substring(0, 150) || "no reasoning"
    } | VaR:${var_bps}bps | ${autonomyLevel}`,
    { nonce: currentNonce }
  );
  await tx1.wait();
  ...
  const tx2 = await registry.validateProposal(
    ...
    validatorConfBps,
    riskScore * 100,
    `[v2] ${decision.validator?.reasoning?.substring(0, 100) || ""} | RWA:${
      allocation.action
    }`,
  ...
  if (
    (decision.consensus || decision.analyst?.confidence >= 0.7) &&
    autonomyLevel === "AUTONOMOUS" &&
    ...
    try {
      ...
      if (swapAmountMNT < 1) {
        console.log(
          `   ⚠️  Swap amount too small (${swapAmountMNT.toFixed(
            2
          )} MNT). Skipping.`
        );
      } else {
        ...
        if (executionResult.executed) {
          ...
          console.log(
            `   Gas: ${executionResult.gasUsed} | Block: ${executionResult.blockNumber}`
          );
          ...
          try {
            ...
            console.log(
              `   ⚠️  [DISCIPLINE] Check failed (non-fatal): ${discErr.message?.slice(
                0,
                80
              )}`
            );
  ...
  console.log(
    `║${pad(
      `  Consensus: ${
        decision.consensus ? "APPROVED ✅" : "BLOCKED ❌"
      }  VaR: ${var_bps} bps`
    )}║`
  );
  ...
  console.log(
    `║${pad(
      `  RWA:       ${
        allocation.action
      }  Target: ${allocation.targetRWAAllocation.toFixed(0)}%`
    )}║`
  );
  console.log(
    `║${pad(
      `  DEX:       1 MNT = $${(mntQuote?.estimatedOut || 0).toFixed(4)}`
    )}║`
  );
  console.log(
    `║${pad(
      `  On-chain:  ${totalApproved} approved / ${totalRejected} rejected`
    )}║`
  );
  console.log(
    `║${pad(
      `  KMS:       ${kmsSig.simulated ? "simulated" : "HARDWARE"}  DER: ${
        kmsSig.derRoundTrip ? "✅" : "mock"
      }`
    )}║`
  );
  console.log(`╚${"═".repeat(boxW)}╝\n`);
