...
class TrajectoryLogger {
  ...
  _load() {
    try { return JSON.parse(fs.readFileSync(TRAJECTORY_PATH, "utf8")); }
    catch { return []; }
  }

  _save() {
    fs.mkdirSync(path.dirname(TRAJECTORY_PATH), { recursive: true });
    fs.writeFileSync(TRAJECTORY_PATH, JSON.stringify(this.trajectories.slice(-200), null, 2));
  }
  ...
  logCycle(trajectory) {
    const entry = {
      ...
      metrics: {
        ...
        totalCycle_ms: (trajectory.analystDuration || 0) + (trajectory.validatorDuration || 0) + (trajectory.onchainDuration || 0),
        // Consistency check
        ...
        dataPointsUsed: this._countDataPoints(trajectory.analyst?.reasoning || ""),
      }
    };
  ...
  _checkConsistency(analyst) {
    ...
    const bullishSignals = ["buy", "accumulate", "breakout", "bullish", "risk-on", "long", "upside"];
    const bearishSignals = ["sell", "de-risk", "bearish", "correction", "defensive", "risk-off", "downside"];

    const bullCount = bullishSignals.filter(s => r.includes(s)).length;
    const bearCount = bearishSignals.filter(s => r.includes(s)).length;

    if (bullCount > bearCount && targetAsset === "mETH" && direction === "risk_on") return "consistent";
    if (bearCount > bullCount && targetAsset === "mUSD" && direction === "risk_off") return "consistent";
  ...
  getProcessMetrics() {
    ...
    const consistencies = recent.map(t => t.metrics?.actionReasoningConsistent).filter(Boolean);
    const cycleTimes = recent.map(t => t.metrics?.totalCycle_ms).filter(t => t > 0);
    const dataPoints = recent.map(t => t.metrics?.dataPointsUsed).filter(d => d > 0);
    ...
    return {
      ...
      consistency: {
        ...
        contradictory: consistencies.filter(c => c === "contradictory").length,
        ambiguous: consistencies.filter(c => c === "ambiguous").length,
        consistencyRate: consistencies.length > 0
          ? (consistencies.filter(c => c === "consistent").length / consistencies.length * 100).toFixed(1) + "%"
          : "N/A",
      },
      timing: {
        avgCycle_ms: cycleTimes.length > 0 ? Math.round(cycleTimes.reduce((a, b) => a + b, 0) / cycleTimes.length) : 0,
      ...
      informationDiversity: {
        avgDataPoints: dataPoints.length > 0 ? (dataPoints.reduce((a, b) => a + b, 0) / dataPoints.length).toFixed(1) : 0,
      },
    };
  }
}
