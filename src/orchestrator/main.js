...
async function runCycle() {
  ...
  try {
    ...
    console.log(
      `      ETH: $${marketData.ethPrice} | Sentiment: ${marketData.sentiment} (${marketData.fearGreedValue}) | mETH Yield: ${marketData.mETHYield}%`
    );
    ...
    console.log(
      `      Action: ${decision.action} | Direction: ${decision.direction} | Confidence: ${decision.confidence}`
    );
    ...
    const shouldExecute =
      ...
      decision.confidence >= config.RISK_PARAMS.minConfidence;
    console.log(
      `[3/4] Should execute: ${shouldExecute} (confidence ${decision.confidence} vs threshold ${config.RISK_PARAMS.minConfidence})`
    );
    ...
    const reasoningHash = JSON.stringify({
      ...
      market: {
        ...
        fgi: marketData.fearGreedValue,
      },
      decision: {
        action: decision.action,
        confidence: decision.confidence,
        target: decision.targetAsset,
      },
    }).substring(0, 200);
    ...
    console.log(
      `      Block: ${receipt.blockNumber} | Gas: ${receipt.gasUsed} | Total decisions: ${totalDecisions}`
    );
    ...
    if (shouldExecute) {
      ...
      console.log(
        `      ⚠️  Swap execution disabled on testnet (no real liquidity)`
      );
    }
