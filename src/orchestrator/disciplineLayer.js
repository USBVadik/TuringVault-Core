...
const WALLET_ADDRESS = process.env.WALLET_ADDRESS || "0xDC783CDBfA993f3FC299460627b204E83bf4fb5a";
...
async function verify(params) {
  ...
  if (params.action === "swap" && params.txHash) {
    try {
      ...
      if (!tx) {
        checks.push({ name: "tx_exists", status: "FAIL", detail: "TX not found on chain" });
        blocked = true;
        blockReason = "Transaction hash not found on Mantle";
        repairStep = "Re-fetch tx hash from wallet; if missing, execution may have failed silently";
      } else {
        checks.push({ name: "tx_exists", status: "PASS", detail: `Block ${tx.blockNumber}` });
        
        // Verify sender matches our wallet
        if (tx.from.toLowerCase() !== WALLET_ADDRESS.toLowerCase()) {
          checks.push({ name: "tx_sender", status: "FAIL", detail: `Expected ${WALLET_ADDRESS}, got ${tx.from}` });
          blocked = true;
          blockReason = "TX sender does not match vault wallet";
          repairStep = "Verify wallet address in .env matches deployer";
        } else {
          checks.push({ name: "tx_sender", status: "PASS", detail: "Matches vault wallet" });
        }
        ...
        if (receipt) {
          ...
          if (confirmations < MIN_CONFIRMATIONS) {
            checks.push({ name: "tx_confirmed", status: "WARN", detail: `Only ${confirmations} confirmations (need ${MIN_CONFIRMATIONS})` });
            // Don't block — just warn, it'll confirm soon
          } else {
            checks.push({ name: "tx_confirmed", status: "PASS", detail: `${confirmations} confirmations` });
          }
          
          // Check tx succeeded
          if (receipt.status === 0) {
            checks.push({ name: "tx_success", status: "FAIL", detail: "TX reverted on-chain" });
            blocked = true;
            blockReason = "Transaction reverted — swap failed";
            repairStep = "Check slippage settings and token allowances";
          } else {
            checks.push({ name: "tx_success", status: "PASS", detail: "TX successful" });
          }
        }
      }
    } catch (err) {
      checks.push({ name: "tx_proof", status: "ERROR", detail: err.message?.slice(0, 100) });
      // Don't block on RPC errors — degrade gracefully
    }
  } else if (params.action === "hold") {
    checks.push({ name: "tx_proof", status: "SKIP", detail: "Hold action — no tx to verify" });
  }
  ...
  if (params.priceTimestamp && params.decisionTimestamp) {
    ...
    if (priceAge > MAX_PRICE_AGE_MS) {
      checks.push({ 
        ...
        status: "FAIL", 
        detail: `Price data was ${(priceAge / 1000).toFixed(0)}s old at decision time (max: ${MAX_PRICE_AGE_MS / 1000}s)` 
      });
      if (!blocked) {
        blocked = true;
        blockReason = "Stale price data used in decision";
        repairStep = "Re-fetch live price and re-evaluate decision with fresh data";
      }
    } else if (priceAge > MAX_PRICE_AGE_MS * 0.8) {
      checks.push({ 
        name: "price_freshness", 
        status: "WARN", 
        detail: `Price data was ${(priceAge / 1000).toFixed(0)}s old — approaching staleness` 
      });
    } else {
      checks.push({ 
        name: "price_freshness", 
    ...
    checks.push({ name: "price_freshness", status: "SKIP", detail: "Timestamps not provided" });
  }
    ...
    if (regimeMismatch) {
      ...
      if (driftStreak >= MAX_DRIFT_STREAK) {
        ...
        if (!blocked) {
          ...
          repairStep = "Re-evaluate market regime; consider forcing HOLD until regime stabilizes";
        }
      } else {
        checks.push({ 
          name: "drift_detection", 
      ...
      driftStreak = 0;
      checks.push({ name: "drift_detection", status: "PASS", detail: `Action aligns with ${params.regime} regime` });
    }
  }
  ...
  console.log(`   ${icon} [DISCIPLINE] ${status}${blocked ? ` — ${blockReason}` : ""}`);
  for (const c of checks) {
    const cIcon = c.status === "PASS" ? "✓" : c.status === "FAIL" ? "✗" : c.status === "WARN" ? "⚠" : "○";
