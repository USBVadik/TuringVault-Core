...
async function runCycle() {
  ...
  if (dailyCycles >= MAX_DAILY_CYCLES) {
    console.log(
      `⚠️  Daily limit reached (${MAX_DAILY_CYCLES} cycles). Pausing until tomorrow.`
    );
    return;
  }
  ...
  console.log(`\n${"═".repeat(60)}`);
  console.log(
    `  CYCLE #${cycleCount} | ${new Date().toISOString()} | Errors: ${consecutiveErrors}/${MAX_CONSECUTIVE_ERRORS}`
  );
  ...
  try {
    ...
    console.log(
      `\n⏱️  Cycle completed in ${elapsed}s | Next in ${
...
║  Interval: ${
    CYCLE_INTERVAL_MS / 1000
  }s | Max errors: ${MAX_CONSECUTIVE_ERRORS} | Daily limit: ${MAX_DAILY_CYCLES}     ║
╚══════════════════════════════════════════════════════════════╝
  `);
