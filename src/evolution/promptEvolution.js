...
const EVOLUTION_LOG_PATH = path.resolve(
  __dirname,
...
class PromptEvolution {
  constructor(options = {}) {
    ...
    this.decisionLog = new ethers.Contract(
  ...
  async getCurrentAgentCard() {
    ...
    if (
      !tokenURI ||
      tokenURI === "" ||
      tokenURI === "ipfs://QmTuringVaultAgent"
    ) {
  ...
  _fallbackCard() {
    ...
    if (fs.existsSync(localAsset)) {
      try {
        return JSON.parse(fs.readFileSync(localAsset));
      } catch {}
    }
    ...
    return {
      ...
      systemPrompt: {
        version,
        analyst:
          "You are TuringVault's AI Analyst (GLM-5). Analyze market data. Output JSON: action, targetAsset, confidence, reasoning.",
        validator:
          "You are TuringVault's Validator (Claude). Score risk 0-100, approve/reject.",
        riskParameters: {
  ...
  async getPerformanceData() {
    ...
    try {
      ...
      totalFeedback = await this.reputation.getTotalFeedback(
        this.config.agentTokenId
      );
    } catch (e) {
      // Contract may not have these exact methods — try alternatives
      console.log(
        `  Note: getAgentScore not available, using event-based approach`
      );
    }
  ...
  shouldEvolve(performance) {
    ...
    if (fs.existsSync(EVOLUTION_LOG_PATH)) {
      ...
      if (lastEvolution) {
        ...
        if (hoursSince < this.config.cooldownHours) {
          return {
            should: false,
            reason: `Cooldown: ${hoursSince.toFixed(
              1
            )}h since last evolution (need ${this.config.cooldownHours}h)`,
          };
        }
      }
    }
    ...
    if (outcomeData && outcomeData.total > 0) {
      const wins =
        (outcomeData.summary.correctBlocks || 0) +
        (outcomeData.summary.goodCalls || 0);
    ...
    if (outcomeData && outcomeData.raw && outcomeData.raw.length >= 10) {
      ...
      for (const entry of chronological) {
        const isHold =
          (entry.action || "").toLowerCase().includes("hold") ||
          entry.outcome === "CORRECT_BLOCK" ||
          entry.outcome === "MISSED_ALPHA";
        if (isHold) {
          consecutiveHolds++;
      ...
      if (consecutiveHolds >= 10 && holdWithBigMove > 0) {
        triggers.push(
          `${consecutiveHolds} consecutive HOLDs with asset moving >3%`
        );
      }
    }
    ...
    if (triggers.length === 0) {
      return {
        should: false,
        reason: `No evolution triggers met (${settledCount} trades settled, performance acceptable)`,
      };
    }

    return {
      should: true,
      reason: `Evolution triggered: ${triggers.join(" | ")}`,
    };
  }
  ...
  async selfReflect(currentCard, performance) {
    const currentAnalystPrompt =
      currentCard?.systemPrompt?.analyst || "No current prompt found";
...
${
  hasRealOutcomes
    ? `REAL OUTCOME HISTORY (last ${outcomeData.total} settled decisions):
- Correct blocks (HOLD, price fell — firewall worked): ${
        outcomeData.summary.correctBlocks
      }
- Missed alpha (HOLD, price rose — too conservative): ${
        outcomeData.summary.missedAlpha
      }
- Good calls (SWAP approved, price in our favour): ${
        outcomeData.summary.goodCalls
      }
- Bad calls (SWAP approved, price against us): ${outcomeData.summary.badCalls}
- Accuracy: ${outcomeData.summary.accuracy}
- Total PnL signal: ${outcomeData.summary.totalPnlBps > 0 ? "+" : ""}${
        outcomeData.summary.totalPnlBps
      } bps

INDIVIDUAL OUTCOMES (most recent first):
${outcomeData.forPrompt
  .map(
    (e) =>
      `  ${e.when} | ${e.action} | consensus=${e.consensus} | ETH ${
        e.pricePct >= 0 ? "+" : ""
      }${e.pricePct}% | ${e.outcome} | score${e.score >= 0 ? "+" : ""}${
        e.score
      }`
  )
  .join("\n")}`
...
${
  hasRealOutcomes &&
  outcomeData.summary.missedAlpha > outcomeData.summary.correctBlocks
...
Output STRICT JSON:
{
  ...
  async applyEvolution(reflection, validation, currentCard) {
    ...
    const updatedCard = {
      ...
      systemPrompt: {
        ...
        version: this.incrementVersion(
          currentCard?.systemPrompt?.version || "2.0.0"
        ),
        lastUpdated: new Date().toISOString(),
        analyst:
          reflection.newAnalystPrompt || currentCard?.systemPrompt?.analyst,
        riskParameters: {
    ...
    const ipfsResult = await pinJSON(
      updatedCard,
      `TuringVault-AgentCard-${updatedCard.systemPrompt.version}`
    );
    console.log(`     CID: ${ipfsResult.cid}`);

    // Update on-chain
    console.log("  ⛓️  Updating tokenURI on-chain...");
    const tx = await this.identity.setAgentURI(
      this.config.agentTokenId,
      ipfsResult.uri
    );
    await tx.wait();
    ...
    const localCardPath = path.resolve(
      __dirname,
  ...
  async evolve(options = {}) {
    ...
    console.log(
      `   Card found: ${hasCard} | Version: ${
        currentCard?.systemPrompt?.version || "none"
      }`
    );
    ...
    console.log(
      `   Score: ${performance.score} | Decisions: ${performance.totalDecisions} | Feedback: ${performance.totalFeedback}`
    );
    ...
    console.log(
      `   Approved: ${validation.approved} | Risk: ${validation.riskAssessment}`
    );
    ...
    const result = await this.applyEvolution(
      reflection,
      validation,
      currentCard
    );
    ...
    console.log(
      `║  Version: ${result.updatedCard.systemPrompt.version.padEnd(
        10
      )}                                   ║`
    );
    console.log(
      `║  CID: ${result.ipfsResult.cid.slice(0, 20)}...                    ║`
    );
