...
describe("Trading Constants", () => {
  describe("sanity checks", () => {
    ...
    it("ELEVATED should be higher than BASE confidence", () => {
      expect(constants.ELEVATED_CONFIDENCE_THRESHOLD).toBeGreaterThan(
        constants.BASE_CONFIDENCE_THRESHOLD
      );
    });
    ...
    it("signal weights should sum to ~1.0", () => {
      const sum =
        constants.FUNDING_WEIGHT +
        constants.REGIME_WEIGHT +
        constants.GRID_WEIGHT +
        constants.NANSEN_WEIGHT;
      expect(sum).toBeCloseTo(1.0, 2);
    });

    it("regime thresholds should be ordered correctly", () => {
      expect(constants.REGIME_RANGING_THRESHOLD).toBeLessThan(
        constants.REGIME_TRENDING_THRESHOLD
      );
    });
  });
