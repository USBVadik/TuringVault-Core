...
describe('normalizeAnalystResponse', () => {
  describe('action normalization', () => {
    it('should accept valid "swap" action', () => {
      const result = normalizeAnalystResponse({ action: 'swap', confidence: 0.8 });
      expect(result.action).toBe('swap');
    });

    it('should accept valid "hold" action', () => {
      const result = normalizeAnalystResponse({ action: 'hold', confidence: 0.3 });
      expect(result.action).toBe('hold');
    });

    it('should normalize "SWAP" to "swap"', () => {
      const result = normalizeAnalystResponse({ action: 'SWAP', confidence: 0.9 });
      expect(result.action).toBe('swap');
    });

    it('should use "decision" field as fallback for action', () => {
      const result = normalizeAnalystResponse({ decision: 'swap', confidence: 0.7 });
      expect(result.action).toBe('swap');
    });

    it('should default to "hold" for unrecognized actions', () => {
      const result = normalizeAnalystResponse({ action: 'buy_everything', confidence: 0.9 });
      expect(result.action).toBe('hold');
    });
  ...
  describe('confidence normalization', () => {
    it('should pass through valid 0-1 confidence', () => {
      const result = normalizeAnalystResponse({ action: 'swap', confidence: 0.85 });
      expect(result.confidence).toBe(0.85);
    });

    it('should convert percentage confidence (85) to decimal (0.85)', () => {
      const result = normalizeAnalystResponse({ action: 'swap', confidence: 85 });
      expect(result.confidence).toBe(0.85);
    });

    it('should clamp confidence above 100 to 1', () => {
      const result = normalizeAnalystResponse({ action: 'swap', confidence: 150 });
      expect(result.confidence).toBe(1);
    });

    it('should clamp negative confidence to 0', () => {
      const result = normalizeAnalystResponse({ action: 'swap', confidence: -0.5 });
      expect(result.confidence).toBe(0);
    });

    it('should use fallback for NaN confidence', () => {
      const result = normalizeAnalystResponse({ action: 'swap', confidence: 'very high' });
  ...
  describe('direction normalization', () => {
    it('should normalize "bullish" to "risk_on"', () => {
      const result = normalizeAnalystResponse({ action: 'swap', confidence: 0.8, direction: 'bullish' });
      expect(result.direction).toBe('risk_on');
    });

    it('should normalize "bearish" to "risk_off"', () => {
      const result = normalizeAnalystResponse({ action: 'swap', confidence: 0.8, direction: 'bearish' });
      expect(result.direction).toBe('risk_off');
    });

    it('should accept "risk_on" directly', () => {
      const result = normalizeAnalystResponse({ action: 'swap', confidence: 0.8, direction: 'risk_on' });
      expect(result.direction).toBe('risk_on');
    });

    it('should use "sentiment" as fallback for direction', () => {
      const result = normalizeAnalystResponse({ action: 'swap', confidence: 0.8, sentiment: 'bullish' });
      expect(result.direction).toBe('risk_on');
    });

    it('should default to "neutral" for unknown directions', () => {
      const result = normalizeAnalystResponse({ action: 'swap', confidence: 0.8, direction: 'sideways_chaos' });
  ...
  describe('targetAsset normalization', () => {
    it('should normalize "ETH" to "mETH"', () => {
      const result = normalizeAnalystResponse({ action: 'swap', confidence: 0.8, targetAsset: 'ETH' });
      expect(result.targetAsset).toBe('mETH');
    });

    it('should normalize "meth" to "mETH"', () => {
      const result = normalizeAnalystResponse({ action: 'swap', confidence: 0.8, targetAsset: 'meth' });
      expect(result.targetAsset).toBe('mETH');
    });
    ...
    it('should use "target_asset" snake_case field', () => {
      const result = normalizeAnalystResponse({ action: 'swap', confidence: 0.8, target_asset: 'mETH' });
      expect(result.targetAsset).toBe('mETH');
    });
  ...
  describe('allocationPct normalization', () => {
    it('should pass through valid allocation', () => {
      const result = normalizeAnalystResponse({ action: 'swap', confidence: 0.8, allocationPct: 30 });
      expect(result.allocationPct).toBe(30);
    });

    it('should clamp allocation above 100', () => {
      const result = normalizeAnalystResponse({ action: 'swap', confidence: 0.8, allocationPct: 200 });
      expect(result.allocationPct).toBe(100);
    });

    it('should clamp negative allocation to 0', () => {
      const result = normalizeAnalystResponse({ action: 'swap', confidence: 0.8, allocationPct: -10 });
    ...
    it('should use "allocation_pct" snake_case field', () => {
      const result = normalizeAnalystResponse({ action: 'swap', confidence: 0.8, allocation_pct: 45 });
  ...
  describe('reasoning normalization', () => {
    it('should pass through reasoning string', () => {
      const result = normalizeAnalystResponse({ action: 'hold', confidence: 0.5, reasoning: 'Market is choppy' });
      expect(result.reasoning).toBe('Market is choppy');
    });

    it('should use "reason" as fallback', () => {
      const result = normalizeAnalystResponse({ action: 'hold', confidence: 0.5, reason: 'Low liquidity' });
      expect(result.reasoning).toBe('Low liquidity');
    });

    it('should truncate reasoning to 1000 chars', () => {
      const longReason = 'x'.repeat(2000);
      const result = normalizeAnalystResponse({ action: 'hold', confidence: 0.5, reasoning: longReason });
      expect(result.reasoning.length).toBe(1000);
    });

    it('should provide default reasoning when missing', () => {
      const result = normalizeAnalystResponse({ action: 'hold', confidence: 0.5 });
  ...
  describe('riskFactors normalization', () => {
    it('should pass through array of risk factors', () => {
      const result = normalizeAnalystResponse({ action: 'swap', confidence: 0.8, riskFactors: ['slippage', 'low volume'] });
      expect(result.riskFactors).toEqual(['slippage', 'low volume']);
    });

    it('should convert string to array', () => {
      const result = normalizeAnalystResponse({ action: 'swap', confidence: 0.8, riskFactors: 'high volatility' });
      expect(result.riskFactors).toEqual(['high volatility']);
    });

    it('should use "risk_factors" snake_case', () => {
      const result = normalizeAnalystResponse({ action: 'swap', confidence: 0.8, risk_factors: ['MEV risk'] });
...
describe('normalizeValidatorResponse', () => {
  describe('approved normalization', () => {
    it('should accept boolean true', () => {
      const result = normalizeValidatorResponse({ approved: true, validatorConfidence: 0.8, riskScore: 40 });
      expect(result.approved).toBe(true);
    });

    it('should accept boolean false', () => {
      const result = normalizeValidatorResponse({ approved: false, validatorConfidence: 0.4, riskScore: 80 });
    ...
    it('should convert string "yes" to boolean true', () => {
      const result = normalizeValidatorResponse({ approved: 'yes', validatorConfidence: 0.8, riskScore: 40 });
      expect(result.approved).toBe(true);
    });

    it('should use "approve" as fallback field', () => {
      const result = normalizeValidatorResponse({ approve: true, validatorConfidence: 0.8, riskScore: 40 });
      expect(result.approved).toBe(true);
    });

    it('should default to false when missing', () => {
      const result = normalizeValidatorResponse({ validatorConfidence: 0.8, riskScore: 40 });
      expect(result.approved).toBe(false);
    });
  });
  ...
  describe('validatorConfidence normalization', () => {
    it('should pass through valid 0-1 confidence', () => {
      const result = normalizeValidatorResponse({ approved: true, validatorConfidence: 0.82, riskScore: 30 });
      expect(result.validatorConfidence).toBe(0.82);
    });

    it('should convert percentage to decimal', () => {
      const result = normalizeValidatorResponse({ approved: true, validatorConfidence: 78, riskScore: 30 });
      expect(result.validatorConfidence).toBe(0.78);
    });

    it('should use "validator_confidence" snake_case', () => {
      const result = normalizeValidatorResponse({ approved: true, validator_confidence: 0.9, riskScore: 30 });
      expect(result.validatorConfidence).toBe(0.9);
    });

    it('should fall back to "confidence" field', () => {
      const result = normalizeValidatorResponse({ approved: true, confidence: 0.75, riskScore: 30 });
      expect(result.validatorConfidence).toBe(0.75);
    });

    it('should use fallback for NaN', () => {
      const result = normalizeValidatorResponse({ approved: true, validatorConfidence: 'high', riskScore: 30 });
  ...
  describe('riskScore normalization', () => {
    it('should pass through valid risk score', () => {
      const result = normalizeValidatorResponse({ approved: true, validatorConfidence: 0.8, riskScore: 55 });
      expect(result.riskScore).toBe(55);
    });

    it('should use "risk_score" snake_case', () => {
      const result = normalizeValidatorResponse({ approved: true, validatorConfidence: 0.8, risk_score: 42 });
  ...
  describe('edge cases', () => {
    ...
    it('should handle complete validator response', () => {
      ...
      expect(result.reasoning).toBe('Funding rate confirms direction, R:R is 2.1:1');
    });
  });
});
