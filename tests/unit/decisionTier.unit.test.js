...
const { classifyDecisionTier, TIERS } = require('../../src/orchestrator/decisionTier');

// Convenience builders so each test reads top-down.
const analyst = (confidence = 0.8) => ({ confidence, action: 'swap', targetAsset: 'mETH' });
...
describe('classifyDecisionTier', () => {
  ...
  test('null analyst → BLOCKED_BY_PARSE_FAILURE', () => {
    expect(classifyDecisionTier(decision({ analyst: null }), market()))
      .toBe(TIERS.BLOCKED_BY_PARSE_FAILURE);
  });

  test('null validator → BLOCKED_BY_PARSE_FAILURE', () => {
    expect(classifyDecisionTier(decision({ validator: null }), market()))
      .toBe(TIERS.BLOCKED_BY_PARSE_FAILURE);
  });

  test('both null → BLOCKED_BY_PARSE_FAILURE', () => {
    expect(classifyDecisionTier({ analyst: null, validator: null }, market()))
      .toBe(TIERS.BLOCKED_BY_PARSE_FAILURE);
  });
  ...
  test('null decision argument → BLOCKED_BY_PARSE_FAILURE', () => {
    expect(classifyDecisionTier(null, null)).toBe(TIERS.BLOCKED_BY_PARSE_FAILURE);
  });

  // ── Low confidence (precedes regime check) ───────────────────────
  test('analyst.confidence below default threshold → LOW_CONFIDENCE', () => {
    expect(classifyDecisionTier(
      decision({ analyst: analyst(0.4) }),
      market(),
  ...
  test('regime HOLD → BLOCKED_BY_REGIME', () => {
    expect(classifyDecisionTier(decision(), market('HOLD')))
      .toBe(TIERS.BLOCKED_BY_REGIME);
  });

  test('regime UNKNOWN → BLOCKED_BY_REGIME', () => {
    expect(classifyDecisionTier(decision(), market('UNKNOWN')))
      .toBe(TIERS.BLOCKED_BY_REGIME);
  });
  ...
  test('validator.approved=false → BLOCKED_BY_VALIDATOR', () => {
    expect(classifyDecisionTier(
      decision({ validator: validator(false) }),
      market(),
  ...
  test('validator.approved missing → BLOCKED_BY_VALIDATOR (treated as false)', () => {
    expect(classifyDecisionTier(
      decision({ validator: { /* no approved field */ } }),
      market(),
    )).toBe(TIERS.BLOCKED_BY_VALIDATOR);
  });
  ...
  test('all green + consensus + swap → EXECUTED_SWAP', () => {
    expect(classifyDecisionTier(decision(), market('TREND_UP')))
      .toBe(TIERS.EXECUTED_SWAP);
  });

  test('contrarian regime still allows EXECUTED_SWAP', () => {
    expect(classifyDecisionTier(decision(), market('CONTRARIAN_LONG')))
      .toBe(TIERS.EXECUTED_SWAP);
  });

  test('RANGING regime still allows EXECUTED_SWAP', () => {
    expect(classifyDecisionTier(decision(), market('RANGING')))
      .toBe(TIERS.EXECUTED_SWAP);
  });
