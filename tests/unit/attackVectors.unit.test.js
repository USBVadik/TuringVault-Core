...
const {
  ...
  ATTACK_TYPES,
  _attacks,
} = require("../../src/orchestrator/attackVectors");
...
describe("attackVectors", () => {
  ...
  describe("applyAttack — control flow", () => {
    ...
    test("throws on unknown type", () => {
      expect(() =>
        applyAttack(BASE_MARKET, "definitely_not_a_real_attack")
  ...
  describe("pump_signal specifics", () => {
    test("default pumps price ~15% and sets euphoric sentiment", () => {
      ...
      expect(out.ethPrice).toBeGreaterThan(BASE_MARKET.ethPrice * 1.1);
  ...
  describe("oracle_conflict specifics", () => {
    ...
    test("preserves other signals (funding, regime)", () => {
      const out = applyAttack(BASE_MARKET, "oracle_conflict");
      expect(out.structuredSignals.regime).toEqual(
        BASE_MARKET.structuredSignals.regime
      );
      expect(out.structuredSignals.signals.funding).toEqual(
        BASE_MARKET.structuredSignals.signals.funding
      );
    });
  });
  ...
  describe("promptContext injection", () => {
    test.each(ATTACK_TYPES)(
      "%s appends [INJECTED] marker to promptContext",
      (type) => {
        const out = applyAttack(BASE_MARKET, type);
