...
| 10  | **Lifetime Decision-Quality / Outcome Score from settled outcomes** (no simulation, no backtest) | [/backtest page](https://frontend-seven-beta-46.vercel.app/backtest) — outcome-score curve built from settled decision outcomes; not wallet PnL; cumulative bps + decision-level table |
...
**Stats (observed 2026-06-11 07:09 UTC — refreshable via `/api/health`, `/api/performance`, and `/api/decisions`; contract counters on Mantlescan are the on-chain ground truth):**

- **455 decision records/proposals** exposed through the public API, with full reasoning pinned off-chain and cryptographically anchored on Mantle (live count: [`/api/decisions`](https://frontend-seven-beta-46.vercel.app/api/decisions) + contract surfaces)
- **123 of 455 ValidationRegistry proposals rejected before execution** (~27.0% rejection rate) — adversarial Validator + confidence gates blocking unsafe proposals; this proposal denominator can differ from DecisionLog rows by one during fresh-cycle writes
- **332 approved, 123 rejected** at the registry level — proposals reaching consensus go on-chain; off-chain block reasons (regime, low confidence, portfolio guard, intent-no-exec) shown in tier breakdown on `/proof-explorer`
...
| **Compliance awareness** | Operator-funded demo capital only; no public deposits, no yield promise, no live USDY claim, and a roadmap for allowlists/KYC/jurisdiction policy before any public vault |
...
| Open-source repo with setup, architecture, deployed addresses | ✅ This README + [`docs/ARCHITECTURE.md`](docs/ARCHITECTURE.md) + contract table |
...
The on-chain `totalRejected` counter on ValidationRegistry reflects proposals blocked by **either** gate. The 2026-06-11 07:09 UTC snapshot is **123 rejected / 455 proposals = 27.0%**, demonstrating the system's capital-preservation bias without implying that every cycle produces a swap. The ratio drifts cycle-to-cycle as market conditions change; the **live ratio** is always the on-chain value (`totalRejected() / totalProposals()` on `0x6841…63b6`), not this README.
...
- **LLM-driven route.** Analyst's action vocabulary includes
...
- **Deterministic idle-parking route.** When the agent has been
...
- [x] On-chain decision logging (455 public decision records/proposals in the 2026-06-11 07:09 UTC snapshot, growing whenever the best-effort GitHub Actions schedule fires)
- [x] Adversarial validation (123 of 455 ValidationRegistry proposals rejected before execution in the 2026-06-11 07:09 UTC snapshot; live count via ValidationRegistry)
