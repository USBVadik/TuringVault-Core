...
| 6   | **Reproducible AI** — replay any past decision against the original LLM provider | [`replay-manifests/`](.kiro/audits/raw/replay-manifests) — every cycle ships a manifest with the exact prompts and raw responses for analyst/validator/arbiter; run `npm run replay <cycle-id>` to re-invoke Bedrock/Vertex and verify outputs match |
