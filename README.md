...
| **Elfa**           | Social intelligence — mindshare, smart-account ratio, attention surge          | [`src/data/elfa.js`](src/data/elfa.js) — wired into `signalEngine.js` as 5th signal + `/api/elfa-snapshot` (V2 paths: `/v2/data/top-mentions`, `/v2/aggregations/trending-tokens`) | ⚠️ Rate-limited — Elfa free-tier monthly quota hit on 2026-05-28. UI on `/social` shows the actual 429 message; cron pipeline gracefully skips the 5th signal. Refreshes on the 1st of each month or via tier upgrade. |
