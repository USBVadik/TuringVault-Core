...
| 6   | **Reproducible AI** — replay any past decision against the original LLM provider, anchor sealed on-chain | [`replay-manifests/`](.kiro/audits/raw/replay-manifests) — every cycle ships a manifest with the exact prompts and raw responses for analyst/validator/arbiter; `combinedAnchor = keccak256(utf8(ipfsCid) ‖ manifestHash)` is written to `DecisionLog.txHash` + `ReputationRegistry.reasoningHash` each cycle so the manifest cannot be edited after the fact without breaking the binding. Run `npm run replay <cycle-id>` to re-invoke Bedrock/Vertex and verify outputs + anchor match. See [audit 18](.kiro/audits/18-onchain-anchor-replay-manifest.md). |
