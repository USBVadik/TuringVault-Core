...
async function main() {
  ...
  console.log(
    `  Est. completion: ${new Date(
      Date.now() + MAX_ITERATIONS * INTERVAL_MS
    ).toISOString()}`
  );
  console.log(
    "═══════════════════════════════════════════════════════════════\n"
  ...
  for (let i = 1; i <= MAX_ITERATIONS; i++) {
    ...
    console.log(
      `  📊 Success: ${successCount}/${i} | Spent: ${ethers.formatEther(
        spent
      )} MNT | Remaining: ${ethers.formatEther(currentBalance)} MNT`
    );
  ...
  console.log(
    `  Gas spent: ${ethers.formatEther(startBalance - finalBalance)} MNT`
  );
