...
const VALIDATION_REGISTRY_ADDRESS = "0x6841d3DAF81A446C8Bd6934F7516f2Ee1b4d63b6";
...
async function fetchLiveStats(provider) {
  const dl = new ethers.Contract(DECISION_LOG_ADDRESS, [
    "function totalDecisions() view returns (uint256)",
  ], provider);

  const vr = new ethers.Contract(VALIDATION_REGISTRY_ADDRESS, [
  ...
  const [totalDecisions, consensusRate, minAnalyst, minValidator, maxRisk] = await Promise.all([
    dl.totalDecisions(),
    vr.getConsensusRate(),
...
function buildAgentCard(stats) {
  return {
    ...
    systemPrompt: {
      ...
      analyst: "You are TuringVault's AI Analyst (GLM-5). Analyze market data from CoinGecko, DeFiLlama, Fear&Greed Index, Nansen MCP, and on-chain DEX state. Output: action (hold/swap/provide_liquidity), targetAsset, confidence (0-1), reasoning (max 200 chars). Be aggressive but data-driven.",
      validator: "You are TuringVault's Validator (Claude 4.6). Review the Analyst's proposal against current market conditions. Score risk 0-100, provide validatorConfidence 0-1, and approve/reject. Protect capital above all. Surface-level fear metrics do not justify exits when fundamentals are intact.",
      riskParameters: {
...
async function uploadAndUpdateAgentCard() {
  ...
  console.log(`   Decisions: ${stats.totalDecisions} | Approved: ${stats.approved} | Blocked: ${stats.rejected}`);
  ...
  const result = await pinJSON(agentCard, `TuringVault-AgentCard-v${stats.totalDecisions}`);
  ...
  const identity = new ethers.Contract(IDENTITY_ADDRESS, [
  ...
  console.log(`  Stats:    ${stats.rejected}/${stats.total} blocked (${stats.blockRate}%)`);
  console.log("═══════════════════════════════════════════════════════════");
