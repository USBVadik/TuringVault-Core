...
- [ ] 2. Create `src/rwa/usdt0Module.js`
    ...
    - Body: mirror of `src/rwa/usdyModule.js` shape but with USDT0
      ...
      `calculateAllocation`.
    - Acceptance:
        - Export `USDT0Module` and `USDT0_ADDRESS`.
        - `getPosition(addr)` returns `{ token, address, balance,
          decimals, totalSupply, apy: 0, underlying, issuer }`.
        - `getContextForAI` does NOT mention an APY for USDT0.
        - Unit test: mocked provider, asserts shape and zero APY.
...
- [ ] 4. Create `src/orchestrator/rwaAllocator.js`
    ...
    - Acceptance:
        - Path A `rwa_allocate` produces `{source:'llm',
          from:'USDT', to:'USDT0'}` when consensus + matching action.
        - Path A `rwa_exit` symmetric (USDT0 → USDT).
        - Path B fires only when consensus=false, FLAT > 24 h,
          regime ≠ TREND_UP, cooldown elapsed,
          parkUsd ≥ MIN_BALANCE_USD.
        - Returns `{skip:true, _gate:'min-balance'}` on dust wallet.
        - Returns `{skip:true, _gate:'daily-cap'}` when 24h spend at
          `MAX_PER_DAY_USD`.
        - CP1–CP4 verified via task 5 unit tests.
...
- [ ] 10. Patch `scripts/run-cycle.js` summary writer
    ...
    - Changes:
        - Pull `result?.rwaResult?.txHash` into `summary.txHashes`.
        - Add `summary.rwa = { source, executed, amountInUsd, from,
