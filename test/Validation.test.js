...
describe("TuringVaultValidation (Pre-Action Checks)", function () {
  ...
  describe("Validation Request", function () {
    it("should create validation request", async function () {
      const requestHash = ethers.keccak256(ethers.toUtf8Bytes("swap_mETH_USDY_0.5"));
      ...
      const event = receipt.logs.find(l => l.fragment?.name === "ValidationRequest");
    ...
    it("should reject self-validation", async function () {
      ...
      await expect(
        validation.validationRequest(owner.address, 0, "ipfs://test", requestHash)
    ...
    it("should reject duplicate requests", async function () {
      ...
      await validation.validationRequest(validator.address, 0, "ipfs://1", requestHash);
      await expect(
        validation.validationRequest(validator.address, 0, "ipfs://2", requestHash)
    ...
    it("should reject from non-owner", async function () {
      ...
      await expect(
        validation.connect(attacker).validationRequest(validator.address, 0, "ipfs://x", requestHash)
      ).to.be.reverted;
    });
  });
  ...
  describe("Validation Response", function () {
    ...
    beforeEach(async function () {
      ...
      await validation.validationRequest(validator.address, 0, "ipfs://QmIntent", requestHash);
    });
    ...
    it("should reject from unauthorized address", async function () {
      await expect(
        validation.connect(attacker).validationResponse(
          requestHash, 90, "ipfs://fake", ethers.ZeroHash, "hack"
        )
    ...
    it("should reject score > 100", async function () {
      await expect(
        validation.connect(validator).validationResponse(
          requestHash, 101, "", ethers.ZeroHash, ""
        )
  ...
  describe("Pre-Action Check (isActionApproved)", function () {
    ...
    beforeEach(async function () {
      ...
      await validation.validationRequest(validator.address, 0, "ipfs://QmCheck", requestHash);
    });

    it("should return NOT approved before response", async function () {
      const [approved, score, expired] = await validation.isActionApproved(requestHash);
    ...
    it("should return APPROVED after high score", async function () {
      await validation.connect(validator).validationResponse(
        requestHash, 80, "", ethers.ZeroHash, "trade"
      );
      const [approved, score, expired] = await validation.isActionApproved(requestHash);
    ...
    it("should return NOT approved after low score", async function () {
      await validation.connect(validator).validationResponse(
        requestHash, 30, "", ethers.ZeroHash, "risky"
      );
      const [approved, score, expired] = await validation.isActionApproved(requestHash);
      expect(approved).to.be.false;
    ...
    it("should return false for non-existent request", async function () {
      ...
      const [approved, score, expired] = await validation.isActionApproved(fakeHash);
      expect(approved).to.be.false;
      expect(score).to.equal(0);
    });
  });
  ...
  describe("Summary", function () {
    it("should return aggregated stats", async function () {
      ...
      for (let i = 0; i < 3; i++) {
        ...
        await validation.validationRequest(validator.address, 0, `ipfs://req${i}`, hash);
        await validation.connect(validator).validationResponse(
          hash, 70 + i * 10, "", ethers.ZeroHash, "trade"
        );
      }
  ...
  describe("Admin", function () {
    it("should authorize/revoke validators", async function () {
      ...
      expect(await validation.authorizedValidators(attacker.address)).to.be.true;
      
      await validation.revokeValidator(attacker.address);
      expect(await validation.authorizedValidators(attacker.address)).to.be.false;
    });
    ...
    it("should reject invalid config", async function () {
      await expect(validation.updateConfig(101, 300)).to.be.revertedWith("Invalid score");
      await expect(validation.updateConfig(50, 30)).to.be.revertedWith("Timeout too short");
    });
  });
});
