...
export async function GET() {
  ...
  try {
    const raw = readFileSync(join(process.cwd(), '..', 'src', 'data', 'outcomes.json'), 'utf-8');
  ...
  const equityCurve: { idx: number; nav: number; bps: number; action?: string; date?: string }[] = [];
  ...
  return NextResponse.json({
    summary: {
      ...
      avgTradeBps: settled.length > 0 ? Math.round(cumulativeBps / settled.length) : 0,
