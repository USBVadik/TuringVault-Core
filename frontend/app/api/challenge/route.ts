...
async function verifyOnChain() {
  try {
    ...
    return {
      ...
      blockRate: Number(totalProposals) > 0
        ? `${Math.round((Number(totalRejected) / Number(totalProposals)) * 100)}%`
        : '0%',
...
async function buildDeterministicResponse(type: string) {
  ...
  if (!payload) {
    return { status: 400, body: { error: 'unknown attack type', available: KNOWN_ATTACKS } };
  }
