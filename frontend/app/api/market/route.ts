...
const ENDPOINTS = {
  COINGECKO_ETH: 'https://api.coingecko.com/api/v3/simple/price?ids=ethereum,mantle&vs_currencies=usd&include_24hr_change=true',
...
export async function GET() {
  try {
    const [priceRes, fgRes, tvlRes] = await Promise.all([
      fetch(ENDPOINTS.COINGECKO_ETH).then(r => r.json()).catch(() => null),
      fetch(ENDPOINTS.FEAR_GREED).then(r => r.json()).catch(() => null),
      fetch(ENDPOINTS.DEFILLAMA_TVL).then(r => r.json()).catch(() => null),
    ]);
