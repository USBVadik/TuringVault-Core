 ...
 */
async function computeRwaPctNav(): Promise<{
  navUsd: number;
  rwaUsd: number;
  pctNav: number;
} | null> {
  try {
    ...
    try {
      ...
      if (r.ok) {
        ...
        if (j["mantle-staked-ether"]?.usd)
          ethPrice = j["mantle-staked-ether"].usd;
      }
    } catch {
      /* fall back to defaults — honest enough for % calc */
    }
...
export async function GET() {
  try {
    ...
    try {
      ...
      if (cgRes.ok) {
        ...
        const channelPosition =
          (currentPrice - support) / (resistance - support);

        // Simple trend detection
        const recentSlice = prices.slice(-12);
        const firstHalf =
          recentSlice.slice(0, 6).reduce((a: number, b: number) => a + b, 0) /
          6;
        const secondHalf =
          recentSlice.slice(6).reduce((a: number, b: number) => a + b, 0) / 6;
    ...
    let riskReward = null;
    if (
      positionState.targetExit &&
      positionState.stopLoss &&
      positionState.entryPrice
    ) {
      const reward = Math.abs(
        positionState.targetExit - positionState.entryPrice
      );
    ...
    const flatSinceMs = positionState?.flatSince
      ? Date.parse(positionState.flatSince)
      : NaN;
