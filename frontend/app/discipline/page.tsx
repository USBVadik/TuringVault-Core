      ...
      <div className="max-w-5xl mx-auto">
        <div className="mb-10">
          ...
          <h1 className="text-3xl font-bold mb-2">
            <span className="text-purple-400">🛡️</span> Discipline Layer
          </h1>
          <p className="text-white/40 text-sm">
            Post-execution proof verification — Synrail-inspired. Every multi-agent cycle passes through three gates:
            <span className="text-white/30"> TX exists on chain · price data &lt; 60s old · action aligns with declared regime.</span>
          </p>
        </div>

        {loading && <div className="p-8 text-center text-white/30 text-sm">Loading history…</div>}
          ...
          <>
            ...
            {data.latest && (
              <div className="p-6 rounded-lg border border-white/[0.06] bg-white/[0.02] mb-6">
                <h3 className="text-xs font-bold text-white/60 uppercase tracking-widest mb-3">Latest Cycle</h3>
                <div className="flex items-center gap-4 mb-3">
                  <span className={`text-base font-bold ${
                    data.latest.status === 'ACCEPTED' ? 'text-emerald-400'
                    : data.latest.status === 'BLOCKED' ? 'text-red-400'
                    : data.latest.status === 'ERROR' ? 'text-yellow-400'
                    : 'text-white/50'
                  }`}>
                    {data.latest.status}
                  </span>
                  {data.latestEntry?.at && (
                    <span className="text-[10px] text-white/30 font-mono">
                      {relativeTime(data.latestEntry.at)} · cycle #{data.latestEntry.decisionId ?? '?'}
                    </span>
                  )}
                </div>
                ...
                <div className="space-y-1">
                  {data.latest.checks.map((c, i) => (
                    <div key={i} className="flex items-center gap-3 text-xs font-mono">
                      <span className={`${statusColor(c.status)} w-4`}>{statusSymbol(c.status)}</span>
                      <span className="text-white/70 w-32">{GATE_LABEL[c.name] ?? c.name}</span>
                      <span className="text-white/40 flex-1">{c.detail ?? ''}</span>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div className="p-6 rounded-lg border border-white/[0.06] bg-white/[0.02]">
              <h3 className="text-xs font-bold text-white/60 uppercase tracking-widest mb-4">History — last {data.history.length} cycles</h3>
              {data.history.length === 0 ? (
                <p className="text-white/30 text-sm">No cycles recorded yet. New cycles auto-populate.</p>
              ) : (
                <table className="w-full text-xs font-mono">
                  <thead>
                    <tr className="text-left text-white/30 border-b border-white/5">
                      ...
                      {KNOWN_GATES.map((g) => (
                        <th key={g} className="py-2 pr-3">{GATE_LABEL[g]}</th>
                      ))}
                  ...
                  <tbody>
                    {data.history.map((e, i) => {
                      const checkByName = Object.fromEntries(e.checks.map((c) => [c.name, c]));
                      ...
                      return (
                        <>
                          ...
                          >
                            <td className="py-2 pr-3 text-white/60">{e.decisionId ?? '?'}</td>
                            <td className="py-2 pr-3 text-white/40">{relativeTime(e.at)}</td>
                            <td className={`py-2 pr-3 ${
                              e.verdict === 'ACCEPTED' ? 'text-emerald-400/80'
                              : e.verdict === 'BLOCKED' ? 'text-red-400/80'
                              : e.verdict === 'ERROR' ? 'text-yellow-400/80'
                              : 'text-white/40'
                            }`}>{e.verdict}</td>
                            {KNOWN_GATES.map((g) => {
                              const c = checkByName[g];
                              return (
                                <td key={g} className={`py-2 pr-3 ${statusColor(c?.status)}`}>{statusSymbol(c?.status)}</td>
                              );
                            })}
                            <td className="py-2 pr-3 text-white/40 truncate max-w-[200px]">{e.blockReason ?? e.error ?? ''}</td>
                          </tr>
                          {isOpen && (
                            <tr key={`drill-${i}`} className="bg-white/[0.01]">
                              <td colSpan={3 + KNOWN_GATES.length + 1} className="py-3 px-3">
                                <pre className="text-[10px] text-white/50 whitespace-pre-wrap leading-relaxed">{JSON.stringify(e, null, 2)}</pre>
                              </td>
                            </tr>
                          )}
                        </>
...
function SummaryCard({ summary }: { summary: Summary }) {
  return (
    <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
      ...
      <Tile label="Accepted" value={String(summary.acceptedCount)} tone="emerald" />
      ...
      {KNOWN_GATES.map((g) => (
        <Tile
          ...
          value={summary.gatePassRates[g] != null ? `${summary.gatePassRates[g]}%` : '—'}
          tone={summary.gatePassRates[g] != null && summary.gatePassRates[g]! >= 90 ? 'emerald' : 'amber'}
        />
      ))}
...
function Tile({ label, value, tone }: { label: string; value: string; tone?: 'emerald' | 'red' | 'amber' | 'muted' }) {
  const colors = {
