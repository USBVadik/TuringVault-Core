    ...
    async function fetchAll() {
      ...
      const entries = await Promise.all(
        tickers.map(async (sym) => {
          try {
            const r = await fetch(`/api/elfa-snapshot?symbol=${sym}`, { cache: 'no-store' });
    ...
    <main className="relative min-h-screen px-6 py-10 max-w-[1100px] mx-auto text-white">
      <header className="mb-8">
        <div className="flex items-center justify-between mb-2">
          <h1 className="text-2xl font-bold tracking-tight">
            Social Attention <span className="text-white/40 font-normal">· Elfa REST v2</span>
          </h1>
          <Link href="/" className="text-xs text-purple-300/70 hover:text-purple-200 underline">
            ← back to dashboard
          </Link>
        </div>
        <p className="text-xs text-white/40 mt-2 max-w-3xl leading-relaxed">
          Live snapshot of the 5th structured signal feeding the agent: mindshare leaderboard rank,
          24h surge, and the breakdown of reposts from smart vs ct (crypto-twitter generic) accounts.
          V2 strips raw tweet content for ToS compliance — sentiment is reported as <code className="font-mono text-white/60">null</code> rather
          than fabricated. Refreshes every 5 min.
        </p>
      ...
      <div className="mb-6 flex items-center gap-2">
        ...
        <span className="text-[10px] text-white/30 ml-2">
          {loading ? 'loading…' : `${tickers.length} ticker${tickers.length === 1 ? '' : 's'} tracked`}
        </span>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {tickers.map((sym) => {
          ...
          if (!d.available) {
            const isMissingKey = /no_api_key|not configured/i.test(d.reason || '');
            return (
              <div
                key={sym}
              ...
              >
                <div className="flex items-center justify-between mb-2">
                  <span className="text-base font-semibold">{sym}</span>
                  <span className="text-[10px] font-mono text-yellow-300/70">unavailable</span>
                </div>
                <p className="text-xs text-white/50 leading-relaxed">
                  {isMissingKey
                    ? 'ELFA_API_KEY not configured on this deployment.'
                    : `Upstream error: ${(d.reason || 'unknown').slice(0, 200)}`}
                </p>
              </div>
            );
          }
            ...
            >
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-3">
                  ...
                  <span
                    className={`px-2 py-0.5 rounded text-[10px] font-mono font-bold ${signalColor(d.signal)}`}
                  >
                    {d.signal}
                  </span>
                  <span className="text-[10px] font-mono text-white/30">
                    strength {d.strength != null ? `${Math.round(d.strength * 100)}%` : '—'}
                  </span>
                </div>
                <span className="text-[10px] font-mono text-white/30">
              ...
              <div className="grid grid-cols-3 gap-3 mb-4">
                <div className="p-2.5 rounded bg-white/[0.03]">
                  <div className="text-[9px] text-white/30 uppercase tracking-wider mb-1">Mindshare</div>
                  <div className="text-lg font-mono font-bold">
                    {d.mindshare != null ? `${Number(d.mindshare).toFixed(2)}%` : '—'}
                  </div>
                  <div className={`text-[10px] font-mono mt-0.5 ${dmsColor}`}>{dmsLabel}</div>
                </div>
                <div className="p-2.5 rounded bg-white/[0.03]">
                  <div className="text-[9px] text-white/30 uppercase tracking-wider mb-1">Rank</div>
                  <div className="text-lg font-mono font-bold">
                    {d.mindshareRank != null ? `#${d.mindshareRank}` : '—'}
                  </div>
                  <div className="text-[10px] font-mono text-white/30 mt-0.5">in trending top 50</div>
                </div>
                <div className="p-2.5 rounded bg-white/[0.03]">
                  <div className="text-[9px] text-white/30 uppercase tracking-wider mb-1">Mentions</div>
                  <div className="text-lg font-mono font-bold">{fmtNum(d.mentionCount)}</div>
                  <div className="text-[10px] font-mono text-white/30 mt-0.5">top {d.mentionCount ?? 0} sampled</div>
                </div>
              </div>

              <div className="grid grid-cols-2 gap-3 mb-3 text-[11px]">
                <div className="p-2 rounded bg-white/[0.02] border border-white/[0.04]">
                  <div className="text-[9px] text-white/30 uppercase tracking-wider mb-0.5">Reposts</div>
                  <div className="font-mono">
                    <span className="text-emerald-400/80">{d.smartReposts ?? 0} smart</span>
                ...
                <div className="p-2 rounded bg-white/[0.02] border border-white/[0.04]">
                  <div className="text-[9px] text-white/30 uppercase tracking-wider mb-0.5">Engagement (avg)</div>
                  <div className="font-mono text-white/70">
                    {fmtNum(d.avgViews)} views · {fmtNum(d.avgLikes)} likes
                  </div>
                  <div className="text-[10px] text-white/40 mt-0.5">across sampled mentions</div>
                </div>
              </div>
      ...
      <div className="mt-10 pt-6 border-t border-white/[0.05] text-[11px] text-white/40 leading-relaxed max-w-3xl">
        <p>
          The agent reads the same payload every cycle (rolled into the analyst's structured-signals prompt
          alongside funding rate, on-chain flow, yield spread, and liquidation map). The classifier turns
          mindshare-surge + smart-share into BULLISH / BEARISH / NEUTRAL; sentiment itself is intentionally
          left null because Elfa V2 strips raw tweet content for ToS compliance and we refuse to fabricate one.
        </p>
        <p className="mt-3">
